...
- Completed research reports (`ResearchRunDetail.report`, markdown text) render via `react-markdown` + `remark-gfm` (added in Phase 4c, not part of the original requested package list but needed since the backend returns raw markdown) — styled with plain Tailwind arbitrary-variant selectors on a wrapper `div` (`[&_h1]:...` etc.) rather than `@tailwindcss/typography`, to avoid adding a whole plugin for one page.
